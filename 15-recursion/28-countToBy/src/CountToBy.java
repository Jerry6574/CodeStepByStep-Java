/*
Solution to: https://www.codestepbystep.com/problem/view/java/recursion/countToBy

Write a recursive method named countToBy that accepts integer parameters n and m and that produces output indicating how to count to n in increments of m separated by commas. For example, to count to 10 by 1 you'd say countToBy(10, 1); The following table shows several other calls and their expected output:
...
public class CountToBy {
	...
	public static void countToBy(int n, int m) throws IllegalArgumentException{
		if(n < 1 || m < 1) {
			throw new IllegalArgumentException("Inputs must be greater than 1. ");
		} else {
			countoByHelper(n-m, m);
			System.out.print(n);
		}
	}
	
	private static void countoByHelper(int n, int m) {
		if(n >= 1) {
			countoByHelper(n-m, m);
			System.out.print(n + ", ");
		}
